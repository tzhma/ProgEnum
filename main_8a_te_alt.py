'''
mp: 1m20s for 5108
mp for 268536 programs: -n 64 3h10m, wt: 204h
'''
import numpy as np
...
num_top_P2s_kept = 5 	# keep all 5 programs of size 2 for eR_alt=.5 & .75 interleaved with 1-mut
progsize_thres = 5 		# 4, 5
nth_found = 1

extension = ('5108','268536')[1]
filename = 'df_pte_' + extension + '_alt'


## load
# para = pickle.load(open('data/df_param', 'rb')).loc[14, ['h','dp','dpm']].values
para = pd.read_pickle('data/df_param').loc[14, ['h','dp','dpm']].values
eR_rand = .5 +.5*para[-1]
...
df = pd.read_pickle('data/df_enumP_para14_alt')
prog_id_excluded = df[(df['progsize']==2)].sort_values('eR', ascending=True).index.values[num_top_P2s_kept:]
