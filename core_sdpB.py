...
def gen_A_B(para, dim_u):
	...
	u_list = [u0] + u_list + [u1]
	#
	A = np.zeros([dim_u, dim_a, dim_o])
	for u,a,o in product(range(dim_u), range(dim_a), range(dim_o)):
		sgn_a, sgn_o = a_list[a], o_list[o]
		# pa_m = (sgn_a==outmap[m])*1
		pa_u = (sgn_a==np.sign(u_list[u]))*1
		po_ua = (1 + sgn_o*(sgn_a*dp*u_list[u] + dpm)) / 2
		pao_u = po_ua * pa_u
		A[u,a,o] = pao_u
	#
	B = np.zeros([dim_u, dim_u, dim_a, dim_o]) # last 4 indices need to align
	for a,o in product(range(dim_a), range(dim_o)):
		sgn_a, sgn_o = a_list[a], o_list[o]
...
def update_X_Y(X0, Y0, A, B):
	...
	AX0 = AX0 / (AX0.sum(0)[None] + 1e-16)
	BAX0 = B[:,:,:,:,None] * AX0[:,None]
	X1 = BAX0.sum(0)#.sum(0)
	# print(X1.shape)
	X1 = X1.reshape( X1.shape[:1] + (np.prod(X1.shape[1:]),) )
	# print(X1.shape)
...
def run_sdpB(p_arr, para, t_iter_max, dim_u=180):
	#
	A, B = gen_A_B(para, dim_u)
	A = sparse.COO(A)
	B = sparse.COO(B)
	X_init = p_arr[:,None]
	# Y_init = (A * p_arr[:,:,None,None]).sum(0).sum(0).flatten()
	Y_init = (A * p_arr[:,None,None]).sum(0).flatten()
	#
	X, Y = X_init, Y_init
	Y_list = [Y_init]
...
def gen_eR_from_Y_list(Y):
	'''
	sanity check:
	generate eR from ao_hist
	'''
	seq_depth = int(np.log2(len(Y))/2)
...
# para = [.34,.3,-.5] #df_task.loc[para_id, ['h','dp','dpm']].values # h,dp,dpm = para
#
# ## RUN
# eR, p_arr, t_iter = run_bdpB(para, dim_u)
# Y_list = run_sdpB(p_arr, para, max_iter_sdp)
# eR_recov = gen_eR_from_Y_list(Y_list[-1])
#
# pickle.dump(Y_list[-1], open('data/Y_B_%s_%s'%(para_id, dim_u), 'wb'))
# eR, eR_recov, Y_list[-1]
