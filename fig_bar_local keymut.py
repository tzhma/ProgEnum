...
plt.title('num_keymut for increasing seq-len thres: %s'%[(pp3<i).sum() for i in range(1,11)])
plt.bar(range(len(pp3[0])), pp3[0])
plt.xticks(range(len(pp3[0])), labels=tars_9, rotation='-45')
plt.ylabel('min seq_len to be distinguished')
plt.savefig('fig/local_keymut.svg')
